...
register_google(key = "<Input Google API KEY>", write = TRUE) 
create_token(
  app = "Miki_Lab1",
  consumer_key = "<Input Twitter API KEY>",
  consumer_secret = "<Input Twitter API SECRET>",
...
allTweets <- read.csv(file="D:/MS/2ndSem/DIC/Lab1Docs/Lab1EDA/Problem3/DIC4.csv", header=TRUE, sep=",")
stateVal <- allTweets[, "stateName"]
stateVal <- table(stateVal)
finalData <-data.frame(result=names(stateVal),NumberofTweets=c(stateVal))
finalData$region <- tolower(finalData$result)
